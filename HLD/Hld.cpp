...
void dfs(int v)
{
  ...
  for(auto u : g[v])
      ...
      if(!vis[u.F])
	{
	  ...
	  size += sub[u.F];
	  edge_val[u.F] = u.S;   //edge_val saves the value of the edge that ends on it.F
	  edge_ind[E[MP(u.F, v)]] = u.F;   //stores the node at which the edge ends;
	}
    }
  sub[v] = size;
  special[v] = ind;
